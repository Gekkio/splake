(function() {
  splake.atmosphere.startServerPush = function(dtid, timeout) {
    var dt = zk.Desktop.$(dtid);
    if (dt._serverpush)
      dt._serverpush.stop();

    var spush = new splake.atmosphere.ServerPush(dt, timeout);
    spush.start();
  };
  splake.atmosphere.stopServerPush = function(dtid) {
    var dt = zk.Desktop.$(dtid);
    if (dt._serverpush)
      dt._serverpush.stop();
  };
  splake.atmosphere.ServerPush = zk.$extends(zk.Object, {
    desktop: null,
    active: false,
    timeout: 300000,
    delay: 1000,
